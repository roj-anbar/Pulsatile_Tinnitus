...
def generate_windows(n_snapshots, window_length, window_overlap_frac):
    """
    ...
    """
    if window_length > n_snapshots:
        return [(0, n_snapshots)]

    step = int(window_length * (1 - window_overlap_frac))
    windows = []
    start = 0
    while start + window_length <= n_snapshots:
    ...
    if windows[-1][1] < n_snapshots:
        windows.append((n_snapshots - window_length, n_snapshots))
...
def hemodynamics(wall_mesh: pv.PolyData,
    ...
    if window_length is None:
        window_length = n_snapshots  # use entire signal

    windows = generate_windows(n_snapshots, window_length, window_overlap_frac)
    print(f"Generated {len(windows)} windows of size {window_length} with {window_overlap_frac*100:.0f}% overlap.\n")
    ...
    print (f'\nFinished writing windowed SPI files to {output_folder}.')
...
def parse_args():
    ...
    ap.add_argument("--window_length", type=int,            help="Size of window (number of snapshots for each window)")
...
def main():
    ...
    hemodynamics(wall_mesh, input_folder, output_folder, case_name = args.case_name,
                 ...
                 window_length = args.window_length, window_overlap_frac = args.window_overlap, with_mean = args.with_mean)
