...
#mpirun -n $num_cores oasis NSfracStep problem=Artery_ramp_unsteady uOrder=$uOrder timesteps=$timesteps_per_cycle period=$period  cycles=$cycles save_frequency=$save_frequency mesh_name=$casename &>> \$log_file
...
if [ \$simdone == "0" ]; then
  if [ \$restart_no -gt 0 ]; then
    if [ -f \$results_folder/data/\$restart_no/incomplete ] || [ $force == "Yes" ]; then
      ...
      echo \$(date) > \$log_file
      mpirun -n $num_cores oasis NSfracStep problem=Artery_ramp_unsteady uOrder=$uOrder timesteps=$timesteps_per_cycle period=$period cycles=$cycles save_frequency=$save_frequency \
