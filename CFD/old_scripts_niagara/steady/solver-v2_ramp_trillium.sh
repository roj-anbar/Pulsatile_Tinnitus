...
#// Original Author: Mehdi Najafi 2018 (mnuoft@gmail.com) and Anna Haley 2022 (ahaley@mie.utoronto.ca) 
#// Modified by: Rojin Anbarafshan (rojin.anbar@gmail.com)
#///////////////////////////////////////////////////////////////
...
#export LD_LIBRARY_PATH=/home/$scinet_user/.conda/envs/$solver_env_name/lib:/scinet/niagara/software/2020a/opt/intel-2020u1-intelmpi-2020u1/boost/1.69.0/lib:$LD_LIBRARY_PATH
  
#submit everything that follows until "EOST" is read at the end of this file
sbatch --time=$estimated_required_time \
       --job-name=$jobname \
       --nodes=1 \
       --ntasks-per-node=$num_cores \
       --partition=$partition  << EOST
#!/bin/bash
...
PATH_JIT_CACHE=/scratch/$scinet_user/PT/.cache
PATH_BSLSOLVER="$BSLSOLVER_HOME"


export APPTAINERENV_DIJITSO_CACHE_DIR="$PATH_JIT_CACHE" 
export APPTAINERENV_PYTHONPATH="${PATH_UFL_SHIM}:${PATH_BSLSOLVER}${PYTHONPATH:+:${PYTHONPATH}}" 
mkdir -p "$APPTAINERENV_DIJITSO_CACHE_DIR"
...
casename_full=$(python "/home/ranbar/BSLSolver/BSLSolver/common/naming.py" "$casename" "$uOrder" "$cycles" "$timesteps_per_cycle")
...
#acs is the total number of saves we are expecting to see, which includes all the cycles we wanted to print (excluding the first cycle)
...
if [ \$simdone == "0" ]; then
  ...
  if [ \$restart_no -gt 0 ]; then
    if [ -f \$results_folder/data/\$restart_no/incomplete ] || [ $force == "Yes" ]; then
      ...
      apptainer exec --env HYDRA_LAUNCHER=fork $BIND_OPTS ~/fenics-legacy-updated2.sif \
      mpirun -n "$NP" oasis NSfracStep problem=Artery_ramp uOrder=$uOrder timesteps=$timesteps_per_cycle period=$period cycles=$cycles save_frequency=$save_frequency \
...
###############################################################################################
# Beginning of the post-processing
###############################################################################################
...
if [ -f \$results_folder/data/\$restart_no/complete ]; then
    ...
    if [ ! -f \$results_folder/*_hemodynamics_w.tec ] || [ "$force" == "Yes" ]; then
        ...
        out=\$((ssh nia-login01 "cd \$SLURM_SUBMIT_DIR; python \$BSLSOLVER_HOME/BSLSolver/Post/hemodynamic_indices_run_me.py \$results_folder -t $post_processing_time_minutes") 2>&1)
        echo "\$out"
    else
...
if [ -f \$results_folder/data/\$restart_no/complete ]; then
    ...
    if [ \$acs -ne \$uc ]; then
      ...
      if [ \$acs -ne \$wc ]; then
        ...
        out=\$((ssh nia-login01 "cd \$SLURM_SUBMIT_DIR; python \$BSLSOLVER_HOME/BSLSolver/Post/wss_run_me.py \$results_folder -t $post_processing_time_minutes")  2>&1)
        echo "\$out"
