...
def assemble_wall_pressure_for_ROI(wall_mesh, shared_pressure_ctype, ROI_center_pid, ROI_radius):
    ...
    ROI_center = wall_mesh.points[ROI_center_pid] # return coordinates of the desired center
...
def average_spectrogram(data, sampling_rate, n_fft, hop_length=None, win_length=None, 
    ...
    freqs, bins, S_ = stft(x=data, **stft_params)
...
def compute_spectrogram_wall_pressure(wall_mesh, shared_pressure_ctype, ROI_center_pid, ROI_radius, spec_quantity, window_size, num_cycles, period_seconds):
    ...
    if spec_quantity == 'pressure':
        ...
        n_snapshots = wall_pressure_ROI.shape[1] # number of snapshots
        ...
        win_length = n_fft
        #if n_fft is None:
        #    n_fft = spectral.shift_bit_length(int(n_samples / 10)) # divide whole signal to 10 windows

        #spec_args = {}
        #spec_args['sampling_rate'] = sampling_rate
        #spec_args['n_fft'] = window_size
...
def hemodynamics(wall_mesh: pv.PolyData,
    ...
    spec_data = compute_spectrogram(group, shared_pressure_ctype, shared_spectrogram_ctype)
    ...
    spec_output_file = Path(output_folder) / f"{case_name}_specP_center{point_ID}_r{radius}_win{window_size}.npz" #eg. specs/PTSeg028_low_pressure_ID=9301_r=0.1.npz
    ...
    title = f'{case_name}_spec{spec_quantity}_pointID={point_ID}_radius={radius}' 
    ...
    print (f'Finished saving spetrograms to {output_folder}.')
...
def parse_args():
    ...
    ap.add_argument("--ROI_center_pid", required=True,      help="Point ID for the center of ROI to compute spectrogram")
    ap.add_argument("--ROI_radius",     required=True,      help="Radius of ROI to compute spectrogram in mm")
...
def main():
    ...
    hemodynamics(wall_mesh, input_folder, output_folder, case_name = args.case_name, n_process = args.n_process, ROI_center = args.ROI_center_pid, ROI_radius = args.ROI_radius)
