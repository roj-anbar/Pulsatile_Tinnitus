...
def assemble_wall_pressure_for_ROI(output_folder_ROIs, wall_mesh, wall_pressure,
...
"""
def average_spectrogram_in_ROI(data,
                        sampling_rate: float,
                        window_type: str = 'hann',
                        window_length: int | None = None,
                        overlap_frac: float = 0.75,
...
def calculate_average_spectrogram_for_one_ROI(output_folder_files,
                                      output_folder_imgs,
                                      output_folder_ROIs,
                                      wall_mesh,
                                      wall_pressure,
    ...
    if spec_quantity == 'pressure':
        ...
        wall_pressure_ROI = assemble_wall_pressure_for_ROI(output_folder_ROIs, wall_mesh, wall_pressure,
...
def compute_and_save_spectrogram_for_all_ROIs(
    ...
    if ROI_center_csv is not None:
        ...
        for i in range(0, len(ROI_centers), 1):
            ...
            #center_tag = f"cx{cx:.2f}cy{cy:.2f}cz{cz:.2f}"
            if ROI_type == "sphere":
                spectrogram_title = f'{case_name}_specP_win{window_length}_overlap{overlap_frac:.2f}_{ROI_tag}_{ROI_type}_r{ROI_radius}' 
            elif ROI_type == "cylinder":
                spectrogram_title = f'{case_name}_specP_win{window_length}_overlap{overlap_frac:.2f}_{ROI_tag}_{ROI_type}_r{ROI_radius}_h{ROI_height}' 
