...
__author__ = "Mehdi Najafi <mnuoft@gmail.com> and Anna Haley <ahaley@mie.utoronto.ca>"
__date__ = "2018-2025"
...
def problem_parameters(commandline_kwargs, NS_parameters, **NS_namespace):
    ...
    if restart_folder:
        ...
        NS_parameters.update(
            ...
            inlet_flow_type = get_cmdarg(commandline_kwargs, 'inlet_flow_type', 'poiseuille'), # Added by Rojin A. (it can be anything from 'poiseuille', 'womersley', 'flat', 'custom')
            dump_stats = 500,
...
def create_bcs(u_, p_, p_1, t, NS_expressions, V, Q, area_ratio, mesh, subdomain_data, 
    ...
    for i in range(id_in_count):
        ...
        if NS_parameters['inlet_flow_type'] == 'womersley': #if fcs_i_filename[0:3] == 'FC_':
            if mpi_rank == 0:
        ...
        # Added by Rojin A.
        elif NS_parameters['inlet_flow_type'] == 'poiseuille':
        ...
        else: #THIS DOES NOT CURRENTLY WORK #NS_parameters['inlet_flow_type'] == 'custom'
            if mpi_rank == 0:
    ...
    if NS_parameters['inlet_flow_type'] == 'womersley': # Added by Rojin A.
...
def temporal_hook(u_, p_, p, q_, V, mesh, tstep, compute_flux,
    ...
    if mpi_rank == 0:
        ...
        if NS_parameters['inlet_flow_type'] == 'womersley':
